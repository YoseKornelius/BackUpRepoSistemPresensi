...
package com.androidfaceauthentication.base;
...
public abstract class VisionProcessorBase<T> implements VisionImageProcessor {
    ...
    @GuardedBy("this")
    protected Bitmap processingImage;
    ...
    @Override
    public synchronized void processByteBuffer(ByteBuffer data, final FrameMetadata frameMetadata,
    ...
    private synchronized void processLatestImage(final GraphicOverlay graphicOverlay) {
        processingImage = BitmapUtils.getBitmap(latestImage, latestImageMetaData);
        processingMetaData = latestImageMetaData;
        if (processingImage != null && processingMetaData != null && !isShutdown) {
            processImage(latestImage, processingMetaData, graphicOverlay);
        }
        latestImage = null;
        latestImageMetaData = null;
    }

    private void processImage(ByteBuffer data, final FrameMetadata frameMetadata,
                              final GraphicOverlay graphicOverlay) {
        long frameStartMs = SystemClock.elapsedRealtime();

        // If live viewport is on (that is the underneath surface view takes care of the camera preview
        // drawing), skip the unnecessary bitmap creation that used for the manual preview drawing.
        Bitmap bitmap = PreferenceUtils.isCameraLiveViewportEnabled(graphicOverlay.getContext())
                ? null
    ...
    public void processImageProxy(ImageProxy image, GraphicOverlay graphicOverlay) {
        ...
        //save processed image as bitmap and its info
        this.processingImage = BitmapUtils.getBitmap(image);
        this.processingMetaData = new FrameMetadata.Builder()
    ...
    //use this if you want to analyze detected frame image
    protected Bitmap getProcessingImage() {
        return processingImage;
    }

    protected FrameMetadata getProcessingMetaData() {
