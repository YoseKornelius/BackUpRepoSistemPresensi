    ...
    # for i in range(0, detections.shape[2]):
        ...
        if confidence > args["confidence"]:
            ...
            if len(face) > 0:
                ...
                # pred_label = np.where(livenessConf > 0.5, 1, 0)
                pred_label = 1 if livenessConf > 0.5 else 0
                frameColor = (0, 0, 0)
                if livenessConf < 0.50:
                    frameColor = (0, 255, 0)
                else:
                    frameColor = (0, 0, 255)
                ...
                cv2.rectangle(frame, (startX, startY), (endX, endY),
                              frameColor, 2)
                _label = le.get(pred_label) + ": {:.4f}".format(livenessConf)
                cv2.putText(frame, _label, (startX, startY - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, frameColor, 2)
                print(_label)
